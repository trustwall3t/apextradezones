...
export const emailTemplates: EmailTemplates = {
	welcome: (name: string, email: string, token: string) => ({
		...
		${emailStyles}
		<div class="container">
			<img src="https://www.finitedome.vercel.app/logo.png" alt="finitedome Logo" class="logo" />
			<h1>Welcome to finitedome, ${name}!</h1>
			<p>Thank you for registering with us. We're excited to have you on board!</p>
			<h4>Your Verification Token is: <strong>${token}</strong></h4>
			<p>Verify your email by clicking the button below:</p>
			<a href="https://finitedome.vercel.app/auth/verify?email=${email}" class="button">Verify Email</a>
	...
	confirmDeposit: (data: { amount: number }) => ({
		...
		<div class="container">
			...
			<div class="footer">
				<p>Best regards,<br>The finitedome Team</p>
			</div>
		</div>
		`,
	}),
...
export const sendEmail = async <T extends keyof EmailData>(
	...
	try {
		...
		const mailOptions = {
			from: `"finitedome" <${
				process.env.EMAIL_FROM || 'noreply@finitedome.com'
			}>`,
			replyTo: `"finitedome Support" <${
				process.env.EMAIL_FROM || 'noreply@finitedome.com'
			}>`,
			to,
			subject,
